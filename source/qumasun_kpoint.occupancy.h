...
inline
void QUMASUN_KPOINT::mSetOccupancy() {
...
*/
inline
void QUMASUN_KPOINT::mSetOccupancyZero() {
    ...
    if (IsRoot(m_ddm_comm)) {//only ddm root//
        ...
        for (int sk = 0; sk < m_num_having_spin_kpoint; ++sk) {
            ...
            double remained_electron = (double)num_electrons / 2.0;
            if (is_spin_on) {
...
inline
void QUMASUN_KPOINT::mSetOccupancyTemperature() {
	const int proc_id = GetProcessID(m_mpi_comm);
