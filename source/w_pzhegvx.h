...
inline
int lapack_PZHEGVX_split(MYCOMPLEX* A, MYCOMPLEX* B, double* eigen_values, MYCOMPLEX* eigen_vectors, int N, const MPI_Comm& mpi_comm, const BlacsGridInfo& blacs_grid) {


	int ibtype = 1;	// 1 indicate "A v = e B v" type problems//
	char  JOBZ = 'V';//((eigen_vectors) ? 'V' : 'N');

	char  RANGE = 'A'; //all eigen values and vectors//
	char  UPLO = 'U';
	...
	//check for num_proc is much larger than matrix size//
	{
		...
		if (nblk <= 0) {  //when matrix size if much smaller than num_procs, call LAPACK on root procs//
			...
			if (proc_id == 0) {
				return lapack_ZHEGVX< MYCOMPLEX>(A, B, eigen_values, eigen_vectors, N);
			} else {
				return 0;
			}
		}
	...
	MYCOMPLEX* local_a = new MYCOMPLEX[local_row_size * local_col_size * 2];
	MYCOMPLEX* local_b = new MYCOMPLEX[local_row_size * local_col_size * 2];
	MYCOMPLEX* local_z = new MYCOMPLEX[local_row_size * local_col_size * 2];
	...
	int lrwork;
	{
		...
		int NQ0 = numroc_(&n, &nblk, &ZERO, &ZERO, &np_cols);
		lwork = n + (2 * NP0 + NQ0 + nblk) * nblk;
		lrwork = 4 * n + std::max(5 * nn, NP0 * NP0 + 2 * nblk * nblk) + (int)ceil((double)n / (double)(np_rows * np_cols)) * nn;
	...
	pzhegvx_(&ibtype, &JOBZ, &RANGE, &UPLO, &n, (lapack_complex_double*)local_a, &ia, &ja, descA,
		(lapack_complex_double*)local_b, &ib, &jb, descB,
		&vl, &vu, &il, &iu, &ABSTOL, &num_found, &num_found_z,
		eigen_values, &orfac, (lapack_complex_double*)local_z, &iz, &jz, descZ,
		(lapack_complex_double*)work, &lwork, rwork, &lrwork, iwork, &liwork, ifail, iclustr, gap, &info);
