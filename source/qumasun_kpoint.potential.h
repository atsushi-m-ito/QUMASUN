...
*/
inline
void QUMASUN_KPOINT::mSetPotential() {
...
inline
void QUMASUN_KPOINT::mSetPotentialVext(RspaceFunc<double>& V, const Nucleus* nuclei, int num_nuclei) {
...
#if 0
inline
void QUMASUN_KPOINT::mSetPotentialVhart(RspaceFunc<double>& Vhart, RspaceFunc<double>& rho) {
	SetPotentialByPoissonFFT(Vhart.Pointer(), rho.Pointer(),
		m_size_x, m_size_y, m_size_z, m_dx, m_dy, m_dz, m_work);
}

/*
* necessary work size is m_size_x * m_size_y * m_size_z * 4;
*/
inline
void QUMASUN_KPOINT::mSetPotentialVlocalFromCorrespondingRho(double* Vext, const double* nucl_rho, double* work) {
	SetPotentialByPoissonFFT(Vext, nucl_rho,
		m_size_x, m_size_y, m_size_z, m_dx, m_dy, m_dz, work);
}
#endif


inline
void QUMASUN_KPOINT::mSetPotentialVxc(RspaceFunc<double>& Vx, RspaceFunc<double>& Vc, RspaceFunc<double>& rho, RspaceFunc<double>& pcc_rho) {
...
inline
void QUMASUN_KPOINT::mSetPotentialVxcSpin(double* Vx_up, double* Vc_up, double* Vx_down, double* Vc_down, const double* rho, const double* pcc_rho, const double* rho_diff) {
	for (size_t i = 0; i < m_size_3d; ++i) {
...
inline
void QUMASUN_KPOINT::mSetPotentialVxc_local(double* Vx, double* Vc, const double* rho, const double* pcc_rho) {
...
inline
void QUMASUN_KPOINT::mSetPotentialVxcSpin_local(double* Vx_up, double* Vc_up, double* Vx_down, double* Vc_down, const double* rho, const double* pcc_rho, const double* rho_diff) {
    const size_t size_3d = ml_grid.Size3D();
    for (size_t i = 0; i < size_3d; ++i) {
        auto ret = Calc_XC_LSDA(rho[i] + pcc_rho[i], rho_diff[i] / (rho[i] + pcc_rho[i]));
        Vx_up[i] = ret.V_x_up;
...
inline
void QUMASUN_KPOINT::mCheckPotentialVhart(RspaceFunc<double>& Vhart, RspaceFunc<double>& rho) {
...
inline
void QUMASUN_KPOINT::mSetLaplasianFFT(RspaceFunc<double>& out, RspaceFunc<double>& in, double coef, double* work) {
