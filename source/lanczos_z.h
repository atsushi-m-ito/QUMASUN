...
inline
int EigenLanczos_z(const int N, const int num_solution, const int size_trimatrix,
	double* eigen_values, std::complex<double>* eigen_vectors,
	OperationA OpeA) {
	...
	for (int t = 0; t < size_trimatrix; ++t) {
		...
		*/
		...
		if (beta > SMALL_LIMIT) {
			...
			for (size_t i = 0; i < N; ++i) {
				p_next[i] = { distribution(mt), distribution(mt) };
			}
			checking = true;
		}
