...
inline
QUMASUN_KPOINT::QUMASUN_KPOINT(const Input& input, const MPI_Comm& mpi_comm_, const int ddm_num[3]) :
...
//	printf("%f, %f, %f\n", m_dkx, m_dx, 1.0/m_dx);
	...
	const int num_procs_spin_kpoint = num_procs / m_num_procs_ddm;

	if ((num_procs / m_num_procs_ddm)* m_num_procs_ddm != num_procs) {
	...
	
	{
		...
		int key_spin_kpoint = proc_id / m_num_procs_ddm;
		const int color_ddm = key_spin_kpoint;
		...
		MPI_Comm_split(m_mpi_comm, color_place, key_spin_kpoint, &m_same_ddm_place_comm);
	...
	m_work_size = LOBPCG::WorkSize_z_multi_mpi(ml_grid.Size3D(), num_solution);
	...
	if (IsRoot(m_ddm_comm)) {
        ...
        //m_work_size = std::max<size_t>(m_work_size, m_size_3d * 6);
	}
...
*/
inline
bool QUMASUN_KPOINT::mCheckConditions() {
...
inline
void QUMASUN_KPOINT::mInitializeBuffer(const Input& input) {
