...
void QUMASUN_MPI::mPrintTime() {
    ...
    double gemm_flops[3] = {
        GemmGFLOPS(num_solution, num_solution, ml_grid.Size3D(), watch.GetCount(17), watch.GetTime(17)),
        GemmGFLOPS(ml_grid.Size3D(), num_solution, num_solution, watch.GetCount(16), watch.GetTime(16)),
        GemmGFLOPS(num_solution, num_solution, num_solution, watch.GetCount(19), watch.GetTime(19))
    };
    ...
    if (watch.GetCount(19) > 0) {
        printf("  DGEMM3(HS2HS)  : %f GFLOPS  (%f GFLOPS/root_proc)\n", total_gemm_flops[2], gemm_flops[2]);
    }
    else {
        printf("  DGEMM3(HS2HS)  : no-called\n");
    }
...
inline
void QUMASUN_MPI::PrintCondition() {
    const bool is_root = IsRoot(m_mpi_comm);
    if (!is_root)return;
    ...
    printf("k-point sample: gamma point only\n");
