...
template <class T>
void Laplasian6th_base(const GridRange& grid, T* out, const T* p, const double coef, const double dx, const double dy, const double dz) {
	...
	for (int iz = 0; iz < size_z; ++iz) {
		...
		for (int iy = 0; iy < size_y; ++iy) {
			...
			for (int ix = 0; ix < size_x; ++ix) {
				...
				auto d2psidx2 = ((p[i + idx_p3] + p[i + idx_m3]) - 13.5 * (p[i + idx_p2] + p[i + idx_m2]) + 135.0 * (p[i + idx_p] + p[i + idx_m]) - 245.0 * p[i]) / (dx * dx);
				d2psidx2 += ((p[i + idy_p3] + p[i + idy_m3]) - 13.5 * (p[i + idy_p2] + p[i + idy_m2]) + 135.0 * (p[i + idy_p] + p[i + idy_m]) - 245.0 * p[i]) / (dy * dy);
				d2psidx2 += ((p[i + idz_p3] + p[i + idz_m3]) - 13.5 * (p[i + idz_p2] + p[i + idz_m2]) + 135.0* (p[i + idz_p] + p[i + idz_m]) - 245.0 * p[i]) / (dz * dz);
				out[i] += coef * d2psidx2 / (90.0);
...
template <class T>
void Laplasian6th_d(const GridRange& grid, T* out, const T* p, const double coef, const double dx, const double dy, const double dz) {
	const int size_x = grid.SizeX();
	const int size_y = grid.SizeY();
	const int size_z = grid.SizeZ();
#pragma ivdep
...
template <class T>
void Laplasian6th_overspace(const GridRange& grid, T* out, const T* src, T* p, const double coef, const double dx, const double dy, const double dz) {
	const int Nx = grid.SizeX();
	const int Ny = grid.SizeY();
	const int Nz = grid.SizeZ();

	
	const double c0 = -245.0 * coef * (1.0 / (90.0 * dx * dx) + 1.0 / (90.0 * dy * dy) + 1.0 / (90.0 * dz * dz));
...
#if 0
				auto d2psidx2 = c3x * ((p[i + idx_p3] + p[i + idx_m3]) - 13.5 * (p[i + idx_p2] + p[i + idx_m2]) + 135.0 * (p[i + idx_p] + p[i + idx_m]) - 245.0 * p[i])
					+ c3y * ((p[i + idy_p3] + p[i + idy_m3]) - 13.5 * (p[i + idy_p2] + p[i + idy_m2]) + 135.0 * (p[i + idy_p] + p[i + idy_m]) - 245.0 * p[i])
					+ c3z * ((p[i + idz_p3] + p[i + idz_m3]) - 13.5 * (p[i + idz_p2] + p[i + idz_m2]) + 135.0 * (p[i + idz_p] + p[i + idz_m]) - 245.0 * p[i]);
				out[io] += d2psidx2;
#else
...
template <class T>
void Laplasian6th_halo_f(const GridRange& grid, T* out, const T* p, double coef, double dx, double dy, double dz) {
...
template <class T>
void Laplasian6th_halo(const GridRange& grid, T* out, const T* src, T* buf_with_halo, const double coef, const double dx, const double dy, const double dz) {
	const int size_x = grid.SizeX();
	const int size_y = grid.SizeY();
	const int size_z = grid.SizeZ();
	...
	Laplasian6th_halo_f(grid, out, buf_with_halo, coef, dx, dy, dz);
...
void Laplasian6th_7_X(int size_x, int size_y, int iy, int iz, T* out, const T* p, const T* m_y_left, const T* m_y_right, const T* m_z_left, const T* m_z_right, double coef, double dx, double dy, double dz) {
	const double c0 = - 245.0 / (dx * dx) - 245.0 / (dy * dy) - 245.0 / (dz * dz);
	const int idy = size_x;
	const int idz = size_x * size_y;

	auto d2dy2 = [&](int64_t i, int ix) {
		if constexpr (MY == 0) {
			return ((p[i - 3*idy] + p[i + 3* idy]) - 13.5 * (p[i - 2* idy] + p[i + 2*idy]) + 135.0 * (p[i - idy] + p[i + idy]) ) / (dy * dy);
		} else if constexpr (MY == 1) {
			return ((p[i - 3 * idy] + m_y_right[ix]) - 13.5 * (p[i - 2 * idy] + p[i + 2 * idy]) + 135.0 * (p[i - idy] + p[i + idy])) / (dy * dy);
		} else if constexpr (MY == 2) {
			return ((p[i - 3 * idy] + m_y_right[ix + size_x]) - 13.5 * (p[i - 2 * idy] + m_y_right[ix]) + 135.0 * (p[i - idy] + p[i + idy])) / (dy * dy);
		} else if constexpr (MY == 3) {
			return ((p[i - 3 * idy] + m_y_right[ix + 2*size_x]) - 13.5 * (p[i - 2 * idy] + m_y_right[ix + size_x]) + 135.0 * (p[i - idy] + m_y_right[ix])) / (dy * dy);
		} else if constexpr (MY == -1) {
			return ((m_y_left[ix + 2 * size_x] + p[i + 3 * idy]) - 13.5 * (p[i - 2 * idy] + p[i + 2 * idy]) + 135.0 * (p[i - idy] + p[i + idy])) / (dy * dy);
		} else if constexpr (MY == -2) {
			return ((m_y_left[ix + 1 * size_x] + p[i + 3 * idy]) - 13.5 * (m_y_left[ix + 2 * size_x] + p[i + 2 * idy]) + 135.0 * (p[i - idy] + p[i + idy])) / (dy * dy);
		} else if constexpr (MY == -3) {
			return ((m_y_left[ix] + p[i + 3 * idy]) - 13.5 * (m_y_left[ix + 1 * size_x] + p[i + 2 * idy]) + 135.0 * (m_y_left[ix + 2 * size_x] + p[i + idy])) / (dy * dy);
		}
	};

	auto d2dz2 = [&](int64_t i, int ix) {
		if constexpr (MZ == 0) {
			return ((p[i - 3*idz] + p[i + 3*idz]) - 13.5 * (p[i - 2*idz] + p[i + 2*idz]) + 135.0 * (p[i - idz] + p[i + idz])) / (dz * dz);
		} else if constexpr (MZ == 1) {
			return ((p[i - 3*idz] + m_z_right[ix]) - 13.5 * (p[i - 2*idz] + p[i + 2*idz]) + 135.0 * (p[i - idz] + p[i + idz])) / (dz * dz);
		} else if constexpr (MZ == 2) {
			return ((p[i - 3*idz] + m_z_right[ix + size_x]) - 13.5 * (p[i - 2*idz] + m_z_right[ix]) + 135.0 * (p[i - idz] + p[i + idz])) / (dz * dz);
		} else if constexpr (MZ == 3) {
			return ((p[i - 3*idz] + m_z_right[ix + 2*size_x]) - 13.5 * (p[i - 2*idz] + m_z_right[ix + size_x]) + 135.0 * (p[i - idz] + m_z_right[ix])) / (dz * dz);
		} else if constexpr (MZ == -1) {
			return ((m_z_left[ix + 2*size_x] + p[i + 3*idz]) - 13.5 * (p[i - 2*idz] + p[i + 2*idz]) + 135.0 * (p[i - idz] + p[i + idz])) / (dz * dz);
		} else if constexpr (MZ == -2) {
			return ((m_z_left[ix + 1 * size_x] + p[i + 3*idz]) - 13.5 * (m_z_left[ix + 2 * size_x] + p[i + 2*idz]) + 135.0 * (p[i - idz] + p[i + idz])) / (dz * dz);
		} else if constexpr (MZ == -3) {
			return ((m_z_left[ix] + p[i + 3*idz]) - 13.5 * (m_z_left[ix + size_x] + p[i + 2*idz]) + 135.0 * (m_z_left[ix + 2 * size_x] + p[i + idz])) / (dz * dz);
		}
	};
	...
	for (int ix = 3; ix < size_x - 3; ++ix) {
		...
		auto d2psidx2 = c0 * p[i];
		d2psidx2 += ((p[i - 3] + p[i + 3]) - 13.5 * (p[i - 2] + p[i + 2]) + 135.0 * (p[i - 1] + p[i + 1])) / (dx * dx);
	...
	}
	{
		...
		auto d2psidx2 = c0 * p[i];
		d2psidx2 += ((p[i - 3 + size_x] + p[i + 3]) - 13.5 * (p[i - 2] + p[i + 2]) + 135.0 * (p[i - 1] + p[i + 1])) / (dx * dx);
		d2psidx2 += d2dy2(i, ix) + d2dz2(i, ix);
		out[i] += coef * d2psidx2 / (90.0);
	...
	}
	{
		...
		auto d2psidx2 = c0 * p[i];
		d2psidx2 += ((p[i - 3 + size_x] + p[i + 3]) - 13.5 * (p[i - 2 + size_x] + p[i + 2]) + 135.0 * (p[i - 1] + p[i + 1])) / (dx * dx);
		d2psidx2 += d2dy2(i, ix) + d2dz2(i, ix);
		out[i] += coef * d2psidx2 / (90.0);
	...
	}
	{
		...
		auto d2psidx2 = c0 * p[i];
		d2psidx2 += ((p[i - 3 + size_x] + p[i + 3]) - 13.5 * (p[i - 2 + size_x] + p[i + 2]) + 135.0 * (p[i - 1 + size_x] + p[i + 1])) / (dx * dx);
		d2psidx2 += d2dy2(i, ix) + d2dz2(i, ix);
		out[i] += coef * d2psidx2 / (90.0);
	...
	}
	{
		...
		auto d2psidx2 = c0 * p[i];
		d2psidx2 += ((p[i - 3] + p[i + 3 - size_x]) - 13.5 * (p[i - 2] + p[i + 2]) + 135.0 * (p[i - 1] + p[i + 1])) / (dx * dx);
		d2psidx2 += d2dy2(i, ix) + d2dz2(i, ix);
		out[i] += coef * d2psidx2 / (90.0);
	...
	}
	{
		...
		auto d2psidx2 = c0 * p[i];
		d2psidx2 += ((p[i - 3] + p[i + 3 - size_x]) - 13.5 * (p[i - 2] + p[i + 2 - size_x]) + 135.0 * (p[i - 1] + p[i + 1])) / (dx * dx);
		d2psidx2 += d2dy2(i, ix) + d2dz2(i, ix);
		out[i] += coef * d2psidx2 / (90.0);
	...
	}
	{
		...
		auto d2psidx2 = c0 * p[i];
		d2psidx2 += ((p[i - 3] + p[i + 3 - size_x]) - 13.5 * (p[i - 2] + p[i + 2 - size_x]) + 135.0 * (p[i - 1] + p[i + 1 - size_x])) / (dx * dx);
		d2psidx2 += d2dy2(i, ix) + d2dz2(i, ix);
		out[i] += coef * d2psidx2 / (90.0);
...
void Laplasian6th_8_Y(int size_x, int size_y, int size_z, int iz, T* out, const T* p, double coef, double dx, double dy, double dz) {

	for (int iy = 3; iy < size_y - 3; ++iy) {
		Laplasian6th_7_X<0, MZ>(size_x, size_y, iy, iz, out, p,
			&p[size_x * (iy - 3 + size_y)], &p[size_x * (iy + 1 - size_y)],
			&p[size_x * size_y * (iz - 3 + size_z)], &p[size_x * size_y * (iz + 1 - size_z)], coef, dx, dy, dz);
	}
	{
		const int iy = 0;
		Laplasian6th_7_X<-3, MZ>(size_x, size_y, iy, iz, out, p,
			&p[size_x * (iy - 3 + size_y)], &p[size_x * (iy + 1 - size_y)],
			&p[size_x * size_y * (iz - 3 + size_z)], &p[size_x * size_y * (iz + 1 - size_z)], coef, dx, dy, dz);
	}
	{
		const int iy = 1;
		Laplasian6th_7_X<-2, MZ>(size_x, size_y, iy, iz, out, p,
			&p[size_x * (iy - 3 + size_y)], &p[size_x * (iy + 1 - size_y)],
			&p[size_x * size_y * (iz - 3 + size_z)], &p[size_x * size_y * (iz + 1 - size_z)], coef, dx, dy, dz);
	}
	{
		const int iy = 2;
		Laplasian6th_7_X<-1, MZ>(size_x, size_y, iy, iz, out, p,
			&p[size_x * (iy - 3 + size_y)], &p[size_x * (iy + 1 - size_y)],
			&p[size_x * size_y * (iz - 3 + size_z)], &p[size_x * size_y * (iz + 1 - size_z)], coef, dx, dy, dz);
	}
...
template <class T>
void Laplasian6th_8(const GridRange& grid, T* out, const T* p, const double coef, const double dx, const double dy, const double dz) {
	const int size_x = grid.SizeX();
	const int size_y = grid.SizeY();
	const int size_z = grid.SizeZ();
	for (int iz = 3; iz < size_z - 3; ++iz) {
		Laplasian6th_8_Y<0>(size_x, size_y, size_z, iz, out, p, coef, dx, dy, dz);
	}
	{
		const int iz = 0;
		Laplasian6th_8_Y<-3>(size_x, size_y, size_z, iz, out, p, coef, dx, dy, dz);

	}
	{
		const int iz = 1;
		Laplasian6th_8_Y<-2>(size_x, size_y, size_z, iz, out, p, coef, dx, dy, dz);

	}
	{
		const int iz = 2;
		Laplasian6th_8_Y<-1>(size_x, size_y, size_z, iz, out, p, coef, dx, dy, dz);
...
/*
* wrapper of the fastest algorithm of 6th order difference equation
* 
*/
template <class T>
void Laplasian6th(const GridRange& grid, T* out, const T* p, double coef, double dx, double dy, double dz) {
	...
	Laplasian6th_halo(grid, out, p, over_data.data(), coef, dx, dy, dz);
}



template <class T, class FUNC_XL, class FUNC_XR, class FUNC_YL, class FUNC_YR, class FUNC_ZL, class FUNC_ZR>
void Laplasian6th_bundle_X(int64_t Ns, int size_x, int size_y, int size_z, T* out, const T* p, FUNC_XL&& func_xl, FUNC_XR&& func_xr, FUNC_YL&& func_yl, FUNC_YR&& func_yr, FUNC_ZL&& func_zl, FUNC_ZR&& func_zr, double coef, double dx, double dy, double dz) {
	const double c0 = -245.0 / (dx * dx) - 245.0 / (dy * dy) - 245.0 / (dz * dz);
...
#pragma ivdep
				for (int n = 0; n < Ns; ++n) {
					...
					d2psidx2 += ((xm3[n] + xp3[n]) - 13.5 * (xm2[n] + xp2[n]) + 135.0 * (xm1[n] + xp1[n])) / (dx * dx);
					d2psidx2 += ((ym3[n] + yp3[n]) - 13.5 * (ym2[n] + yp2[n]) + 135.0 * (ym1[n] + yp1[n])) / (dy * dy);
					d2psidx2 += ((zm3[n] + zp3[n]) - 13.5 * (zm2[n] + zp2[n]) + 135.0 * (zm1[n] + zp1[n])) / (dz * dz);
					out[n + i] += coef * d2psidx2 / (90.0);
				}
			}
		}
	}
...
template <class T, class FUNC_XL, class FUNC_XR, class FUNC_YL, class FUNC_YR, class FUNC_ZL, class FUNC_ZR>
void Laplasian6th_bundle_X1c(int64_t Ns, int size_x, int size_y, int size_z, T* out, const T* p, FUNC_XL&& func_xl, FUNC_XR&& func_xr, FUNC_YL&& func_yl, FUNC_YR&& func_yr, FUNC_ZL&& func_zl, FUNC_ZR&& func_zr, double coef, double dx, double dy, double dz) {
	auto pow2 = [](T a) {return a * a; };
	const double c0 = -245.0 * coef * (1.0 / (dx * dx) + 1.0 / (dy * dy) + 1.0 / (dz * dz)) / (90.0);
	const double cx1 = coef / (90.0*dx*dx);
	const double cx2 = -13.5 * coef / (90.0 * dx * dx);
	const double cx3 = 135.0 * coef / (90.0 * dx * dx);
	const double cy1 = coef / (90.0 * dy * dy);
	const double cy2 = -13.5 * coef / (90.0 * dy * dy);
	const double cy3 = 135.0 * coef / (90.0 * dy * dy);
	const double cz1 = coef / (90.0 * dz * dz);
	const double cz2 = -13.5 * coef / (90.0 * dz * dz);
	const double cz3 = 135.0 * coef / (90.0 * dz * dz);
...
template <class T, class FUNC_XL, class FUNC_XR, class FUNC_YL, class FUNC_YR, class FUNC_ZL, class FUNC_ZR>
void Laplasian6th_bundle_X3(int64_t Ns, int size_x, int size_y, int size_z, T* out, const T* p, FUNC_XL&& func_xl, FUNC_XR&& func_xr, FUNC_YL&& func_yl, FUNC_YR&& func_yr, FUNC_ZL&& func_zl, FUNC_ZR&& func_zr, double coef, double dx, double dy, double dz) {
	const double c0 = - 245.0 / (dx * dx) - 245.0 / (dy * dy) - 245.0 / (dz * dz);
...
#pragma ivdep
				for (int n = 0; n < Ns; ++n) {
					const auto d2psidx2 = c0 * p0[n]
						+ ((xm3[n] + xp3[n]) - 13.5 * (xm2[n] + xp2[n]) + 135.0 * (xm1[n] + xp1[n])) / (dx * dx);
					//d2psidx2 += ((ym3[n] + yp3[n]) - 13.5 * (ym2[n] + yp2[n]) + 135.0 * (ym1[n] + yp1[n])) / (dy * dy);
					//d2psidx2 += ((zm3[n] + zp3[n]) - 13.5 * (zm2[n] + zp2[n]) + 135.0 * (zm1[n] + zp1[n])) / (dz * dz);
					out[n + i] += coef * d2psidx2 / (90.0);
				}
...
#pragma ivdep
				for (int n = 0; n < Ns; ++n) {
					//auto d2psidx2 = c0 * p0[n];
					//d2psidx2 += ((xm3[n] + xp3[n]) - 13.5 * (xm2[n] + xp2[n]) + 135.0 * (xm1[n] + xp1[n])) / (dx * dx);
					const auto d2psidx2 = ((ym3[n] + yp3[n]) - 13.5 * (ym2[n] + yp2[n]) + 135.0 * (ym1[n] + yp1[n])) / (dy * dy);
					//d2psidx2 += ((zm3[n] + zp3[n]) - 13.5 * (zm2[n] + zp2[n]) + 135.0 * (zm1[n] + zp1[n])) / (dz * dz);
					out[n + i] += coef * d2psidx2 / (90.0);
				}
			}
...
#pragma ivdep
				for (int n = 0; n < Ns; ++n) {
					...
					//d2psidx2 += ((xm3[n] + xp3[n]) - 13.5 * (xm2[n] + xp2[n]) + 135.0 * (xm1[n] + xp1[n])) / (dx * dx);
					//d2psidx2 += ((ym3[n] + yp3[n]) - 13.5 * (ym2[n] + yp2[n]) + 135.0 * (ym1[n] + yp1[n])) / (dy * dy);
					const auto d2psidx2 = ((zm3[n] + zp3[n]) - 13.5 * (zm2[n] + zp2[n]) + 135.0 * (zm1[n] + zp1[n])) / (dz * dz);
					out[n + i] += coef * d2psidx2 / (90.0);
				}
			}
		}
...
template <class T, class FUNC_XL, class FUNC_XR, class FUNC_YL, class FUNC_YR, class FUNC_ZL, class FUNC_ZR>
void Laplasian6th_bundle_X3c(int64_t Ns, int size_x, int size_y, int size_z, T* out, const T* p, FUNC_XL&& func_xl, FUNC_XR&& func_xr, FUNC_YL&& func_yl, FUNC_YR&& func_yr, FUNC_ZL&& func_zl, FUNC_ZR&& func_zr, double coef, double dx, double dy, double dz) {
	auto pow2 = [](T a) {return a * a; };
	const double c0 = -245.0 * coef * (1.0/ (dx * dx) + 1.0/ (dy * dy) + 1.0/ (dz * dz)) / (90.0);
	const double cx1 = coef / (90.0*dx*dx);
	const double cx2 = -13.5 * coef / (90.0 * dx * dx);
...
template <class T, class FUNC_XL, class FUNC_XR, class FUNC_YL, class FUNC_YR, class FUNC_ZL, class FUNC_ZR>
void Laplasian6th_bundle_X4(int64_t Ns, int size_x, int size_y, int size_z, T* out, const T* p, FUNC_XL&& func_xl, FUNC_XR&& func_xr, FUNC_YL&& func_yl, FUNC_YR&& func_yr, FUNC_ZL&& func_zl, FUNC_ZR&& func_zr, double coef, double dx, double dy, double dz) {
	auto pow2 = [](T a) {return a * a; };
	const double c0 = -245.0 * coef * (1.0 / (dx * dx) + 1.0 / (dy * dy) + 1.0 / (dz * dz)) / (90.0);
	const double cx1 = coef / (90.0 * dx * dx);
	const double cx2 = -13.5 * coef / (90.0 * dx * dx);
...
//periodic//
template <class T>
void Laplasian6th_bundle(const GridRange& grid, int64_t Ns, T* out, const T* p, const double coef, const double dx, const double dy, const double dz) {
	const int size_x = grid.SizeX();
	const int size_y = grid.SizeY();
	const int size_z = grid.SizeZ();

	
	Laplasian6th_bundle_X1c<T>(Ns, size_x, size_y, size_z, out, p, 
...
*/
template <class T>
void Laplasian6th_MPI_1D_overspace(const GridRangeMPI& grid, T* out, const T* src, T* buf_with_halo, double coef, double dx, double dy, double dz) {
...
*/
template <class T>
void Laplasian6th_MPI_3D_overspace(const GridRangeMPI& grid, T* out, const T* src, T* buf_with_halo, double coef, double dx, double dy, double dz) {
...
template <class T>
void Laplasian6th_ddm(const GridRangeMPI& grid, T* out, const T* src, double coef, double dx, double dy, double dz) {
	...
	case 1:
		Laplasian6th_MPI_1D_overspace(grid, out, src, buf_with_halo.data(), coef, dx, dy, dz);
		break;
	case 2:
		Laplasian6th_MPI_3D_overspace(grid, out, src, buf_with_halo.data(), coef, dx, dy, dz);
		break;
	case 3:		
		Laplasian6th_MPI_3D_overspace(grid, out, src, buf_with_halo.data(), coef, dx, dy, dz);
		break;
	default:
		Laplasian6th_halo(grid, out, src, buf_with_halo.data(), coef, dx, dy, dz);
		break;
	}
}
