...
inline
void QUMASUN_SINGLE::mInitializeState() {
...
#elif 1
	{
		...
		
		{
			...
			for (int n = 0; n < num_solution; ++n) {
				...
				vecmath::Normalize(psi_r.Pointer(), m_size_3d, dVol);

				const double rnd = distribution(mt);
				val += rnd;
		...
		if (is_spin_on) {
			double val = 1.0;
			for (int n = 0; n < num_solution; ++n) {
				RspaceFunc<double>& psi_r = m_psi_set[n + num_solution];
				RspaceFunc<double>& c_psi_r = m_psi_set[n];

				for (size_t i = 0; i < m_size_3d; ++i) {
					psi_r[i] = c_psi_r[i];// distribution(mt);
				}
				vecmath::Normalize(psi_r.Pointer(), m_size_3d, dVol);
			...
			GramSchmidt(m_size_3d, num_solution, &m_psi_set[num_solution], dVol);
		}
...
inline
void QUMASUN_SINGLE::mInitializeDensity() {
