...
inline
int lapack_PDSYEVX(double* A, double* eigen_values, double* eigen_vectors, int N, const MPI_Comm& mpi_comm) {
	...
	double vu = 1000.0;
	int il = 1; int iu = N;
	const double ABSTOL = 1.0e-14;
	const double orfac = 1.0;

	int lwork = -1;  //if lwork ==0, work is automatically allocated in scalapack routine//
	int liwork = -1;
	...
	info = 0;
	// get working size
	pdsyevx_(&JOBZ, &RANGE, &UPLO, &n, local_a, &ia, &ja, descA,
		&vl, &vu, &il, &iu, &ABSTOL, &num_found, &num_found_z,
		eigen_values, &orfac, local_z,&iz, &jz, descZ, &work_size, &lwork, &iwork_size, &liwork, ifail, iclustr, gap, &info);

	printf("pdsyevx_ [%d/%d contxt=%d]: work_size=%d, iwork_size=%d, info=%d\n", proc_id, num_procs, icontxt, (int)work_size, iwork_size, info);

	printf("local_size[%d/%d contxt=%d]: %d x %d\n", proc_id, num_procs, icontxt, local_row_size, local_col_size); fflush(stdout);

	lwork = (int)work_size ;
	liwork = iwork_size ;
	double* work = new double[(int)work_size];
