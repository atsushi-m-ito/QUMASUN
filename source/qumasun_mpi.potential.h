...
*/
inline
void QUMASUN_MPI::mSetPotential() {
    ...
    if (m_hamiltonian_type == HAMILTONIAN::KohnSham_PP) {//DFT//
	    ...
	    if (is_spin_on && (m_num_spin == 1)) {
		    ...
		    if (proc_id < num_procs / 2) {
			    MPI_Send(ml_Vtot_down, local_size, MPI_DOUBLE, proc_id+ num_procs / 2, TAG, m_mpi_comm);
		    } else{
			    MPI_Status status;
                MPI_Recv(ml_Vtot, local_size, MPI_DOUBLE, proc_id - num_procs / 2, TAG, m_mpi_comm, &status);
		    }
	    }
...
inline
void QUMASUN_MPI::mSetPotentialVext(RspaceFunc<double>& V, const Nucleus* nuclei, int num_nuclei) {
...
#if 0
inline
void QUMASUN_MPI::mSetPotentialVhart(RspaceFunc<double>& Vhart, RspaceFunc<double>& rho) {
	SetPotentialByPoissonFFT(Vhart.Pointer(), rho.Pointer(),
		m_size_x, m_size_y, m_size_z, m_dx, m_dy, m_dz, m_work);
}

inline
void QUMASUN_MPI::mSetPotentialVlocalFromCorrespondingRho(RspaceFunc<double>& Vext, RspaceFunc<double>& nucl_rho) {
...
inline
void QUMASUN_MPI::mSetPotentialVxc(RspaceFunc<double>& Vx, RspaceFunc<double>& Vc, RspaceFunc<double>& rho, RspaceFunc<double>& pcc_rho) {
...
inline
void QUMASUN_MPI::mSetPotentialVxcSpin(double* Vx_up, double* Vc_up, double* Vx_down, double* Vc_down, const double* rho, const double* pcc_rho, const double* rho_diff) {
	for (size_t i = 0; i < m_size_3d; ++i) {
...
inline
void QUMASUN_MPI::mSetPotentialVxc_local(double* Vx, double* Vc, const double* rho, const double* pcc_rho) {
...
inline
void QUMASUN_MPI::mSetPotentialVxcSpin_local(double* Vx_up, double* Vc_up, double* Vx_down, double* Vc_down, const double* rho, const double* pcc_rho, const double* rho_diff) {
    const size_t size_3d = ml_grid.Size3D();
    for (size_t i = 0; i < size_3d; ++i) {
        auto ret = Calc_XC_LSDA(rho[i] + pcc_rho[i], rho_diff[i] / (rho[i] + pcc_rho[i]));
        Vx_up[i] = ret.V_x_up;
...
inline
void QUMASUN_MPI::mCheckPotentialVhart(RspaceFunc<double>& Vhart, RspaceFunc<double>& rho) {
...
inline
void QUMASUN_MPI::mSetLaplasianFFT(RspaceFunc<double>& out, RspaceFunc<double>& in, double coef, double* work) {
...
#undef USE_R2C_C2R
	...
	*/
	RspaceFunc<double>& rho_d = work_rd;
